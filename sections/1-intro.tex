...
without performing the intended task~\cite{rewardhacking}---is a fundamental challenge in reinforcement learning (RL). 
...
NOTE: We start by motivating with RL, so we need to justify why we don't do full RL/
why our preliminary results show promise.
...
they can only check a finite subset of inputs. 
This makes them susceptible to reward hacking, as prior work has shown. [TODO: cite]
...
Conversely, \emph{formal specifications} can quantify over all possible inputs, 
mathematically describing the expected program properties. 
This enables them to provide guarantees over all possible program behaviors. 
% Unlike testing,
