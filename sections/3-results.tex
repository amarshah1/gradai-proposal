...
\subsection{Experiment: In-Context Learning} 
Rather than attempting each task with the same few-shot examples,
each time we encounter an example of a reward hack, we will add it to our ``bank'' of examples.
Thus, as we iterate further through the benchmark, 
we hypothesize that we will see a greater percentage of reward hacks.

\subsection{Experiment: Increasing Model Size} 
We hypothesize that increasing the model size will increase the 
percentage of reward-satisfying code completions, since using a more powerful model
