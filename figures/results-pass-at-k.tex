...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  % Branch B's pie occupies a larger fraction of its source image than Branch
  % A's (its legend is one row instead of two), so we scale it down so the
  % rendered pie diameters match.
  \includegraphics[width=0.71\linewidth]{figures/branch_b_pass_at_k.png}
